    ...
    .then(res => {
        videosLoadedPromise.then(_ => {
            ...
            .then(x => {
                ...
                if (ref.match(/^https?:\/\/([^\/]+\.)?youtube\.com\//i)) {
                    let videoId
                    try {
                        videoId = new URL(ref).searchParams.get("v");
                    } catch (e) {
                    }
                    ...
                    if (video) {
                        _.uniq(video.games
                            ...
                            suggestionElement.className = `badge text-bg-info m-1`
                            suggestionElement.textContent = `Player (any side) ${player}`
                            suggestionElement.onclick = _ => {
                                const filter = {
                                    playerName: player,
                                    playerResult: ""
                                }
                                filters.anySide.push(filter)
                                applyFilters(true)
                            }
                            document.getElementById('youtubeToastBody').appendChild(suggestionElement)
                        })

                        if (video.games[0] && video.games[0].white && video.games[0].black) {
                            game = video.games[0]
                            let suggestionElement = document.createElement("span")
                            suggestionElement.className = `badge text-bg-info m-1`
                            suggestionElement.textContent = `${game.white} vs ${game.black}`
                            suggestionElement.onclick = _ => {
                                filters.white.push({
                            ...
                            let rematchSuggestionElement = document.createElement("span")
                            rematchSuggestionElement.className = `badge text-bg-info m-1`
                            rematchSuggestionElement.textContent = `${game.black} vs ${game.white}`
                            rematchSuggestionElement.onclick = _ => {
                                filters.white.push({
                                    playerName: game.black,
                                    playerResult: ""
                                })
                            ...
                            document.getElementById('youtubeToastBody').appendChild(rematchSuggestionElement)
                        }

                        const toast = new bootstrap.Toast(document.getElementById('youtubeToast'))
                        toast.show()
                    }
                }
            })
