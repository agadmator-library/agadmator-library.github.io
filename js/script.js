...
function applyFilters(shouldPushHistory) {
    ...
    filteredVideos = videos
        ...
        .filter(video => !movesCountFrom || _.some(video.games, game => game.movesCount && game.movesCount >= movesCountFrom))
        .filter(video => !movesCountTo || _.some(video.games, game => game.movesCount && game.movesCount <= movesCountTo))
        .filter(video => !yearFrom || _.some(video.games, game => game.year && game.year >= yearFrom))
        .filter(video => !yearTo || _.some(video.games, game => game.year && game.year <= yearTo))
