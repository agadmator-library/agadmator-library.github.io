...
function test() {
    database.getAllIds().forEach(id => {
        let descriptionGames = database.readDescriptionGames(id);
        descriptionGames.forEach(game => {
            if (game.fen) {
                let initial = game.fen
                let shrinked = fenShortener.shrink(initial)
                let expanded = fenShortener.expand(shrinked)
                console.log(`${shrinked} & ${expanded}`)
                if (expanded !== initial) {
                    throw initial
                }
            }
